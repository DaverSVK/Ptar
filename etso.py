...
def fcn_simSch_max(t_start, T_s, finalIndex, sig_u_ext):
    ...
    for idx in range(1, int(finalIndex)):
        ...
        alpha_e = (sig_u_ext[idx-1,:]-x_log[idx-1,0])*alpha
        e_derivated = x_log[idx-1,1]*-1
        aplpha_e_derivated = e_derivated*alpha
        saturation = (0.068*(100*(e_derivated+alpha_e)+aplpha_e_derivated)-e_derivated)/0.0883;
        if saturation > M:
            u_log[idx-1,:]=1*M
        elif saturation < -M:
            u_log[idx-1,:]=-1*M
